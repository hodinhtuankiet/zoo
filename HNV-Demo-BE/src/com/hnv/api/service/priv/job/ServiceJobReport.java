...
//			return false;
	...
	static {
		...
		filter.add(ViMatMaterialDyn.class.getSimpleName()+"."+TaAutUser.ATT_T_PASS_01);
		filter.add(ViMatMaterialDyn.class.getSimpleName()+"."+TaAutUser.ATT_T_PASS_02);
		filter.add(ViMatMaterialDyn.class.getSimpleName()+"."+TaAutUser.ATT_T_PASS_03);
	}
	//---------------------------------------------------------------------------------------------------------
...
//			ViMatMaterialDyn.doBuildAvatar(list);
//		}
	...
	private static void doLst(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception  {
		...
		List<ViMatMaterialDyn> 	list = reqLst(user, json); //and other params if necessary
		if (list==null ){
			API.doResponse(response,DefAPI.API_MSG_KO);
			return;
		}
	...
	private static List<ViMatMaterialDyn> reqLst(TaAutUser user, JSONObject json) throws Exception  {
		...
		List<ViMatMaterialDyn>	list		= ViMatMaterialDyn.DAO.reqList(0, nbLine, cri);

		if (wAvatar) 		TaTpyDocument	.reqBuildAvatar(list, DefDBExt.ID_TA_AUT_USER, ViMatMaterialDyn.ATT_O_AVATAR);
		return list;
	}
...
//				"iTotalDisplayRecords"		, iTotalDisplayRecords,
	...
	public static ResultPagination reqListSearch(TaAutUser user, JSONObject json) throws Exception {
		...
		if(rs == null || hardLoad) {
			...
			List<ViMatMaterialDyn> list 		= ViMatMaterialDyn.DAO.reqList(begin, number);
			
//			ViMatMaterialDyn.doBuildAvatar(list);
//			if(buildInfo) {
//				ViMatMaterialDyn.doBuildPer(list);
//			}
			
			if (total == 0 ) {
				total						= ViMatMaterialDyn.DAO.reqCount().intValue();
			}
	...
	private static Criterion reqRestrictionSearch(TaAutUser user,  String searchKey) {
		...
		Criterion cri = Restrictions.eq(ViMatMaterialDyn.ATT_I_PER_MANAGER, user.reqPerManagerId());
...
//			
////			if(typ01 == ViMatMaterialDyn.TYPE_01_AGENT) {
////				cri = Restrictions.and(	cri, Restrictions.eq(ViMatMaterialDyn.ATT_I_TYPE_02 , typ02));
////			}
//		}
		...
		if (searchKey != null) {
			for (String s : searchKey) {
			    cri = Restrictions.and(cri, Restrictions.or(
			    	Restrictions.ilike(ViJobReportDyn.ATT_I_ID, '%' + s + '%'),
			        Restrictions.ilike(ViJobReportDyn.ATT_I_INFO_01, '%' + s + '%'),
			        Restrictions.ilike(ViJobReportDyn.ATT_I_INFO_02, '%' + s + '%')
			    ));
			}
...
//		if (manId!=null) cri = Restrictions.and(cri, Restrictions.eq(ViMatMaterial.ATT_I_PER_MANAGER, manId));
	...
	//	case 1: colName = ViJobReportDyn.ATT_T_NAME_01; break;	
	//	case 2: colName = ViJobReportDyn.ATT_T_NAME_02; break;		
		}
...
//		}
	...
	private static TaMatMaterial reqNew(TaAutUser user,  JSONObject json) throws Exception {
		...
		attrUsr.put(TaMatMaterial.ATT_D_DATE_02		, null);
		attrUsr.put(TaMatMaterial.ATT_I_AUT_USER_01	, userId);
