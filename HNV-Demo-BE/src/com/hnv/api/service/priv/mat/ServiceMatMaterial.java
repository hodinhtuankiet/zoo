...
//			}
	...
	private static Criterion reqRestriction(TaAutUser user,  Set<String> searchKey, Integer manId, Set<Integer> stat01 ) throws Exception {	
		...
		Criterion cri = Restrictions.in(ViMatMaterialDyn.ATT_I_STATUS_01, stat01);
...
//		}
		...
		if (searchKey != null) {
			for (String s : searchKey) {
			    cri = Restrictions.and(cri, Restrictions.or(
			        Restrictions.ilike(ViMatMaterialDyn.ATT_T_NAME_01, '%' + s + '%'),
			        Restrictions.ilike(ViMatMaterialDyn.ATT_T_NAME_02, '%' + s + '%')
			    ));
			}
