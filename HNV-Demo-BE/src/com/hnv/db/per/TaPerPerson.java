...
public class TaPerPerson extends EntityAbstract<TaPerPerson> {
	...
	public static TaPerPerson reqCheck(Integer modeCheck, Integer uId, Integer perId, JSONObject per, Integer entId) throws Exception {
		...
		if(modeCheck == DefAPI.SV_MODE_NEW) {
			attr.put(ATT_I_AUT_USER_01	, uId);
			attr.put(ATT_I_TYPE_01		, TYP_01_NATURAL);
			attr.put(ATT_I_TYPE_02		, TYP_02_CLIENT);
			attr.put(ATT_I_STATUS_01	, STAT_01_ACTIVE_LV_01);
			attr.put(ATT_D_DATE_01		, new Date());
			entPer	= new TaPerPerson(attr);
			
			TaPerPerson.DAO.doPersist(entPer);
			
			TaAutUser uEnt = TaAutUser.DAO.reqEntityByID(entId);
			...
			attr.put(ATT_I_AUT_USER_02, uId);
	...
	public static void doBuildAvatarForList(List<TaPerPerson>  list) throws Exception {
		...
		List<TaTpyDocument> 	docs 	= TaTpyDocument.reqTpyDocuments (DefDBExt.ID_TA_PER_PERSON, ids, TaTpyDocument.TYPE_01_FILE_MEDIA, TaTpyDocument.TYPE_02_FILE_IMG_AVATAR, null, null);
	...
	public void doBuildAvatar(boolean forced) {
		...
		try {
			this.O_Avatar = TaTpyDocument.DAO.reqEntityByValues(
					TaTpyDocument.ATT_I_ENTITY_TYPE	, ENT_TYP,
