...
public class ViPerPersonDyn extends EntityAbstract<ViPerPersonDyn> {
	...
	public static void doBuildAvatar(List<ViPerPersonDyn> list) throws Exception {
		Set<Integer> ids = ToolSet.reqSetInt(list, ViPerPersonDyn.ATT_I_ID);
		if(!ids.isEmpty()) {
			Map<Integer, TaTpyDocument> mapAva = new HashMap<>();
			List<TaTpyDocument> avas = TaTpyDocument.reqTpyDocuments (DefDBExt.ID_TA_PER_PERSON, ids, TaTpyDocument.TYPE_01_FILE_MEDIA, TaTpyDocument.TYPE_02_FILE_IMG_AVATAR, null, null);
			
			if(avas != null && avas.size() > 0) {
				for(TaTpyDocument a : avas) {
					mapAva.put((Integer) a.req(TaTpyDocument.ATT_I_ENTITY_ID), a);
				}
				for(ViPerPersonDyn u : list) {
